...
--*/
{
	...
	try {
		...
		// DbgPrint("FileMonitor: DriverEntry 2\n");
...
#pragma warning(pop)
		...
		// DbgPrint("FileMonitor: DriverEntry 3 (%ls)\n", DriverObject->DriverName.Buffer);
		...
		status = FltRegisterFilter(DriverObject,
			&FilterRegistration,
			&MiniSpyData.Filter);
		// DbgPrint("FileMonitor: DriverEntry status %lX\n", status);
		if (!NT_SUCCESS(status)) {
			leave;
		}

		// DbgPrint("FileMonitor: DriverEntry 4\n");
		...
		// DbgPrint("FileMonitor: DriverEntry 5\n");
		...
		// DbgPrint("FileMonitor: DriverEntry 6\n");
		...
		// DbgPrint("FileMonitor: DriverEntry 7\n");
		...
		//DbgPrint("FileMonitor: DriverEntry 8\n");
	}
	finally {

		if (!NT_SUCCESS(status)) {

			if (NULL != MiniSpyData.ServerPort) FltCloseCommunicationPort(MiniSpyData.ServerPort);

			if (NULL != MiniSpyData.Filter) FltUnregisterFilter(MiniSpyData.Filter);
...
--*/
{
	...
	if (Data->Iopb->MajorFunction != IRP_MJ_CREATE) return returnStatus;

	//if (Data->Iopb->TargetFileObject->WriteAccess & (FILE_WRITE_DATA | FILE_APPEND_DATA)) {}
	if (FlagOn(Data->Iopb->Parameters.Create.SecurityContext->DesiredAccess, FILE_CREATE_PIPE_INSTANCE | FILE_ADD_FILE | FILE_WRITE_DATA | FILE_APPEND_DATA | FILE_WRITE_EA)) {}
