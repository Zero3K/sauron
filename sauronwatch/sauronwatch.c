...
int main(int argc, char *argv[])
{
	...
	*/
	...
	/* Bucle infinit: comprova cada segon que el servei Sauron estigui en execuci� */
	...
	while (1) {
		...
		/* Si el servei Sauron est� en execuci�, esperem 1 segon i repetim la comprovaci� */
		if (ServiceStatus.dwCurrentState == SERVICE_RUNNING) {
		  Sleep(1000); // 1000 ms
		  continue;
		}

		/* Si el servei no est� en execuci�, l'aturem */
		...
		// Baixar-se el fitxer /sauron/files/ (cont� la llista de fitxers)
		...
		if (get_https(&size, log_server_address, log_server_port, log_server_method, log_server_resource, buffer) == 0) {
			...
			// Rec�rrer la llista de fitxers, per cadascun mirar si el tenim
